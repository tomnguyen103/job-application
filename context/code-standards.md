...
- Store owner-scoped metadata in `tailored_resumes`, including InsForge `storage_key` plus an authenticated `storage_url` route or signed URL, and the PDF in the documented InsForge Storage path.
...
- `@insforge/sdk` — InsForge SDK and SSR auth helpers
